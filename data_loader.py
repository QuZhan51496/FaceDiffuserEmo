...
class Dataset(data.Dataset):
    def __init__(self, json_paths, data_root, data_type="train"):
        ...
        for json_path in json_paths:
            ...
            for video_name, details in data_dict.items():
                if "speaker_id" not in details:
                    continue
                emo, _, level, _  = video_name.split("_")
                if emo != "5" and level != "3":
                    continue
                ...
                emotion_id = details["emotion_id"]
                self.data.append((video_name, audio_path, vertices_path, template_path, spk_id, frames_num, emotion_id))
    ...
    def __getitem__(self, idx):
        video_name, audio_path, vertices_path, template_path, spk_id, frames_num, emotion_id = self.data[idx]
        template_path = os.path.join(os.path.dirname(template_path), "5_level.ckpt")

        # 加载和处理数据
        vertices = np.load(vertices_path)
        temp = dict(torch.load(template_path))
        temp = temp["5_level"]["verts"].reshape((-1))
        ...
        return torch.FloatTensor(audio_segment), crop_vertices, torch.FloatTensor(temp), torch.FloatTensor(one_hot), key, emotion_id
