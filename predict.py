...
import gc
import pyrender
from models import FaceDiff, FaceDiffBeat, FaceDiffDamm, MyFaceDiff_conv, MyFaceDiff_trans_v3
...
def ddim(args, model, model_kwargs, shape, device):
    ...
    vertices = torch.randn(shape, device=device, dtype=torch.float)
    vertices = vertices * scheduler.init_noise_sigma

    scheduler.set_timesteps(1)
    timesteps = scheduler.timesteps.to(device, non_blocking=True)
    ...
    for i, t in enumerate(tqdm(timesteps)):
        with torch.no_grad():
            vertices_pred = model(vertices, t, **model_kwargs)
        vertices = scheduler.step(vertices_pred, t, vertices, **extra_step_kwargs).prev_sample
    return vertices
...
def test_model(args):
    ...
    if args.guidance_uncondp > 0:
        audio_mask = torch.rand(speech_array.shape[0]) < args.guidance_uncondp
        speech_array[audio_mask] = 0
    ...
    # )
    # if args.guidance_scale != 1:
    #     prediction_nocond = diffusion.ddim_sample_loop(
    #         model,
    #         (1, num_frames, args.vertice_dim),
    #         clip_denoised=False,
    #         model_kwargs={
    ...
    # render mesh
    # save_path = f"demo/vis_mesh/{args.model_name.split('/')[1]}"
    # subprocess.call(f"python run_audio2mesh.py --save_path {save_path} --audio_path {wav_path} --verts_path {os.path.join(args.result_path, out_file_name)}.npy", shell=True)
...
def main():
    ...
    parser.add_argument("--guidance_uncondp", type=float, default=0.0)
