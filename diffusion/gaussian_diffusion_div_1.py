...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        with open("/mnt/mnt-quick/PixelAI/Private/heding/FaceDiffuser/voice_dataset_root/pretrained_models/Flame2020/FLAME_masks.pkl", 'rb') as file:
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            ...
            lip_loss = 0.5 * self.l2_loss(lip_out_1, lip_gt) + 0.5 * self.l2_loss(lip_out_2, lip_gt)
            face_loss = 0.5 * self.l2_loss(model_output_1, target) + 0.5 * self.l2_loss(model_output_2, target)
            diversity_loss = F.cosine_similarity(model_output_1_flat, model_output_2_flat, dim=1)
            diversity_loss = diversity_loss.mean()
            terms["loss"] = lambda_face * face_loss + lambda_diversity * diversity_loss
        else:
            raise NotImplementedError(self.loss_type)

        return terms, face_loss, diversity_loss
