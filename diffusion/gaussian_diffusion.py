...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        with open("voice_dataset_root/pretrained_models/Flame2020/FLAME_masks.pkl", 'rb') as file:
            mask = pickle.load(file, encoding='latin1')

        lip_mask = torch.tensor(mask["lips"], dtype=torch.long)
        lip_mask = lip_mask.unique()
        all_indices = torch.arange(5023).long()
        non_lip_mask = ~torch.isin(all_indices, lip_mask)
        non_lip_mask = all_indices[non_lip_mask]
        self.lip_mask = lip_mask
        self.non_lip_mask = non_lip_mask
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            ...
            lambda_l2 = 1.0
            lambda_sth = 1.0
            l2_loss = lambda_l2 * self.l2_loss(target, model_output)
            sth_loss = lambda_sth * self.l2_loss(gt_vel, pred_vel)
            terms["loss"] = l2_loss + sth_loss
        else:
            raise NotImplementedError(self.loss_type)

        return terms, l2_loss, sth_loss
    ...
    #     x_t_1 = self.q_sample(x_start, t, noise=noise_1)
    #     x_t_2 = self.q_sample(x_start, t, noise=noise_2)

    #     terms = {}

    #     if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
    #         model_output_1 = model(x_t_1, self._scale_timesteps(t), **model_kwargs)
    #         model_output_2 = model(x_t_2, self._scale_timesteps(t), **model_kwargs)

    #         target = {
    #             ModelMeanType.PREVIOUS_X: self.q_posterior_mean_variance(
    #                 x_start=x_start, x_t=x_t_1, t=t
    #             )[0],
    #             ModelMeanType.START_X: x_start,
    #             ModelMeanType.EPSILON: noise,
    #         }[self.model_mean_type]

    #         if target.shape != model_output_1.shape:
    #             min_length = min(target.shape[1], model_output_1.shape[1])
    #             model_output_1 = model_output_1[:, :min_length]
    #             target = target[:, :min_length]

    #         model_output_1_flat = model_output_1.squeeze(0)
    #         model_output_2_flat = model_output_2.squeeze(0)
    #         lambda_lip = 0.5
    #         lambda_face = 0.05
    #         lambda_diversity = 0.00002

    #         model_output_1_vert = model_output_1_flat.view(-1, 5023, 3).permute(0, 2, 1)
    #         model_output_2_vert = model_output_2_flat.view(-1, 5023, 3).permute(0, 2, 1)
    #         target_vert = target.squeeze(0).view(-1, 5023, 3).permute(0, 2, 1)
    #         lip_mask = self.lip_mask.to(x_start.device)
    #         non_lip_mask = self.non_lip_mask.to(x_start.device)

    #         lip_out_1 = torch.index_select(model_output_1_vert, dim=-1, index=lip_mask)
    #         face_out_1 = torch.index_select(model_output_1_vert, dim=-1, index=non_lip_mask)
    #         lip_out_2 = torch.index_select(model_output_2_vert, dim=-1, index=lip_mask)
    #         face_out_2 = torch.index_select(model_output_2_vert, dim=-1, index=non_lip_mask)
    #         lip_gt = torch.index_select(target_vert, dim=-1, index=lip_mask)
    #         face_gt = torch.index_select(target_vert, dim=-1, index=non_lip_mask)
    #         lip_out_1 = rearrange(lip_out_1, 'a b c -> a (c b)')
    #         lip_out_2 = rearrange(lip_out_2, 'a b c -> a (c b)')
    #         face_out_1 = rearrange(face_out_1, 'a b c -> a (c b)')
    #         face_out_2 = rearrange(face_out_2, 'a b c -> a (c b)')
    #         lip_gt = rearrange(lip_gt, 'a b c -> a (c b)')
    #         face_gt = rearrange(face_gt, 'a b c -> a (c b)')

    #         lip_loss = self.l2_loss(lip_out_1, lip_gt) + self.l2_loss(lip_out_2, lip_gt)
    #         face_loss = self.l2_loss(face_out_1, face_gt) + self.l2_loss(face_out_2, face_gt)
    #         diversity_loss = F.cosine_similarity(face_out_1, face_out_2, dim=1)
    #         diversity_loss = diversity_loss.mean()
    #         terms["loss"] = lambda_lip * lip_loss + lambda_face * face_loss + lambda_diversity * diversity_loss
    #     else:
    #         raise NotImplementedError(self.loss_type)

    #     return terms, lambda_lip * lip_loss, lambda_face * face_loss, lambda_diversity * diversity_loss
