...
from models import FaceDiff, FaceDiffBeat, FaceDiffDamm, MyFaceDiff, MyFaceDiff_conv, MyFaceDiff_trans_v3
from utils import *
import json
import random
...
def trainer_diff(args, train_loader, dev_loader, model, diffusion, optimizer, epoch=100, device="cuda"):
    ...
    for e in range(epoch + 1):
        ...
        for i, (audio, vertice, template, one_hot, file_name, emotion_id) in pbar:
            iteration += 1
            vertice = vertice.view(vertice.shape[0], vertice.shape[1], -1)
            ...
            if args.guidance_uncondp > 0:
                audio_mask = torch.rand(audio.shape[0]) < args.guidance_uncondp
                audio[audio_mask] = 0
            template, one_hot = template.to(device=device), one_hot.to(device=device)

            loss, l2_loss, sth_loss = diffusion.training_losses(
                model,
                x_start=vertice,
                t=t,
                model_kwargs={
                    ...
                    "one_hot": one_hot,
                    "template": template,
                    "emo_id": emotion_id.to(device=device),
                }
            )
            ...
            pbar.set_description(
                "(Epoch {}, iteration {}) TRAIN LOSS:{:.8f} l2_loss:{:.8f} sth_loss:{:.8f}".format(
                    (e + 1), 
                    iteration, 
                    np.mean(loss_log),
                    l2_loss,
        ...
        if e == args.max_epoch or e % 1 == 0 and e != 0:
...
def main():
    ...
    #         num_layers=args.gru_layers,
    #     )
    dataset, args.train_subjects = get_dataloaders(args)
    spkid_dict = {}
    for i, spk_id in enumerate(args.train_subjects.split(" ")):
        spkid_dict[spk_id] = i
    save_path = os.path.join(args.save_path)
    os.makedirs(save_path, exist_ok=True)
    with open(os.path.join(save_path, "spkid_dict.json"), "w", encoding="UTF-8") as f:
        json.dump(spkid_dict, f, ensure_ascii=True, indent=4)

    model = MyFaceDiff_trans_v3(
            args,
            vertice_dim=args.vertice_dim,
