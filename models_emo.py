...
from utils import PeriodicPositionalEncoding
...
class MyFaceDiff(nn.Module):
    def __init__(self, args, vertice_dim: int, latent_dim: int = 512, cond_feature_dim: int = 1536, diffusion_steps: int = 500, period: int = 30):
        super().__init__()
        self.i_fps = args.input_fps # audio fps (input to the network)
        self.o_fps = args.output_fps # 4D Scan fps (output or target)
